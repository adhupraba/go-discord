...
const MembersModal: FC = () => {
  ...
  const { server } = data as { server: TServerWithMembers };

  const onRoleChange = async (memberId: string, role: TMemberRole) => {
    try {
      setLoadingId(memberId);

      const { data } = await webAxios.patch<TApiData<TServerWithMembers>>(
        `/member/${memberId}?serverId=${server?.id}`,
        { role }
      );

      router.refresh();
      onOpen("members", { server: data.data });
    } catch (err) {
      console.error(err);
    } finally {
      setLoadingId("");
  ...
  const onKick = async (memberId: string) => {
    try {
      ...
      const { data } = await webAxios.delete<TApiData<TServerWithChannelsAndMembers>>(
        `/member/${memberId}?serverId=${server?.id}`
      );
        ...
        <ScrollArea className="mt-8 max-h-[420px] pr-6">
          {server?.members?.map((member) => (
            <div key={member.id} className="flex items-center gap-x-2 mb-6">
              ...
              {loadingId === member.id && <Loader2 className="animate-spin text-zinc-500 ml-auto h-4 w-4" />}
            </div>
          ))}
        </ScrollArea>
      </DialogContent>
